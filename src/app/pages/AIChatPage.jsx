...
export function AIChatPage() {
  ...
  const sendPrompt = useCallback(
    async (userPrompt, priorMessages) => {
      ...
      try {
        ...
        const aiResponse = { role: "assistant", content: responseText };
  ...
  const handleSend = async () => {
    ...
    if (!selectedRepo) {
      setError(
        Object.assign(new Error("Please select a repository from the left sidebar to analyze."), {
          status: -1,
          retryable: false,
        })
      );
            ...
            <div className="space-y-2 overflow-y-auto flex-1 pr-1">
              ...
              {repositories.length === 0 && (
                <div className="text-xs text-muted-foreground text-center py-6">
                  ...
                  <Link to="/upload" className="text-primary block mt-2 hover:underline">
                    Upload a codebase
                  </Link>
                </div>
              )}
      ...
      <div className="w-80 shrink-0">
        <Card className="h-full">
          <CardContent className="pt-6">
            <h3 className="font-semibold mb-4 text-slate-200">Current Context</h3>
            {selectedRepo ? (
              <div className="space-y-4">
                ...
                <div>
                  <div className="text-xs text-muted-foreground mb-1">Files Analyzed</div>
                  <div className="font-medium text-slate-200">
                    {selectedRepo.files.toLocaleString()} files
                  </div>
                </div>

                {selectedRepo.technologies && selectedRepo.technologies.length > 0 && (
                  <div>
                    <div className="text-xs text-muted-foreground mb-2">Technologies</div>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedRepo.technologies.slice(0, 5).map((tech, idx) => (
                        <Badge key={idx}>{tech}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                {selectedRepo.frameworks && selectedRepo.frameworks.length > 0 && (
                  <div>
                    <div className="text-xs text-muted-foreground mb-2">Frameworks</div>
                    <div className="flex flex-wrap gap-1.5">
                      {selectedRepo.frameworks.slice(0, 5).map((fw, idx) => (
                        <Badge key={idx} variant="info">{fw}</Badge>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div>
                  <div className="text-xs text-muted-foreground mb-2">AI Agent Status</div>
                  <ApiStatusDot status={apiStatus} />
                </div>

                {/* Retry shortcut in context panel when offline */}
                {apiStatus === "offline" && lastFailedPrompt && (
                  <motion.div
                  ...
                  >
                    <p className="text-[10px] text-slate-500 mb-2 leading-relaxed">
                      Last prompt failed. You can retry or use locally cached data.
                    </p>
                    <div className="flex flex-col gap-2">
                      <button
                        type="button"
                        onClick={handleRetry}
                        className="flex items-center justify-center gap-1.5 text-[11px] font-semibold px-3 py-2 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/25 transition-all cursor-pointer w-full"
                      >
                        <RefreshCw className="w-3 h-3" />
                        Retry Request
                      </button>
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 text-[11px] font-semibold px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 transition-all cursor-pointer w-full"
                      >
                        <Database className="w-3 h-3" />
                        Use Local Analysis
                      </button>
              ...
              <div className="text-xs text-muted-foreground text-center py-12">
                No active repository context. Select a project from the sidebar.
              </div>
            )}
          </CardContent>
        </Card>
